...
about_text = (
    "\nWe created RESPAN to fill a need for a comprehensive, accurate, automated pipeline that makes use of state-of-the-art tools in a single easy-to-use package."
    "\nEmploying different tools can make analysis pipelines like this convoluted, and it can be challenging to remember all the steps and parameters needed for optimal results."
    "Managing different environments, tracking inputs and outputs from different software, and then interacting with a variety of software with different GUI or command line experiences, "
    "or steps that require extra troubleshooting can easily occupy days to weeks of time. We hope that by incorporating GUIs to train the models for restoration and "
    "segmentation, that it will broadly empower you to consider these approaches when facing other challenging imaging projects and research questions in the future."
    "\n "
    "\nIf you use RESPAN for your research, please cite the following paper:\n\n"
    "RESPAN: an accurate, unbiased and automated pipeline for analysis of dendritic morphology and dendritic spine mapping\n"
...
from PyQt5.QtWidgets import (QApplication, QMainWindow, QVBoxLayout, QTabWidget,
                             ...
                             QMessageBox, QTextEdit, QWidget, QFileDialog, QFormLayout,
                             QHBoxLayout, QGroupBox, QProgressBar, QSplashScreen, QFrame)
...
####### GLOBAL PARAMS
global_GUI_app = False  # set to True when building
global_conda_path = None
...
def resource_path(relative_path):
    """ Get the absolute path to the resource, works for dev and when deployed using PyInstaller """
    try:
        # PyInstaller creates a temp folder and stores path in _MEIPASS
        base_path = sys._MEIPASS
    except Exception:

        dev_base_path = os.path.dirname(__file__)  # directory of the current script
        # base_path = os.path.abspath(os.path.join(dev_base_path, "../External_Scripts"))  # navigate to External_Scripts
        # Append '_internal' only when using _MEIPASS to ensure it looks in the right temporary directory
    if hasattr(sys, '_MEIPASS'):
        return os.path.join(base_path, '_internal', relative_path)
    else:
        #
...
def resource_path(relative_path):
    """ Get the absolute path to the resource, works for development and when deployed using PyInstaller """
    try:
        # When running from PyInstaller-created bundle, use sys._MEIPASS
        base_path = sys._MEIPASS
    except AttributeError:
        # When running in a development environment, use the directory of this script
...
def run_external_script(script_path, conda_path, conda_env, args_dict, logger):
    args_str = ' '.join(f'--{k} "{v}"' for k, v in args_dict.items())

    # Normalize the script path
    script_path = os.path.normpath(script_path)
    # Specify the full path to the conda executable
    conda_executable_path = os.path.normpath(os.path.join(conda_path, "Scripts/conda"))
    # command = f'"{conda_executable_path}" run -n {conda_env} python "{script_path}" {args_str}'
    # Execute the command and capture output in real-time
    command = f'"{conda_executable_path}" run -n {conda_env} python -u "{script_path}" {args_str}'
    # Execute the command and capture output in real-time
...
class EnvSetup(QThread):
    ...
    def run(self):
        global global_conda_path
        conda_paths = self.find_conda_paths()
        respan_found = False
        for conda_path in conda_paths:
            envs = self.get_conda_envs(conda_path)
            if self.respan_env in envs:
                respan_found = True
                global_conda_path = conda_path
                # self.logger.info(global_conda_path)
                self.finished_signal.emit(True, f"\nRESPAN environment is available in {conda_path}.")
                return

        if not respan_found:

            if conda_paths:
                self.create_respan_env(self.respan_env, conda_paths[0])  # Use the first valid Conda path found
                self.finished_signal.emit(True,
                                          f"RESPAN environment {conda_paths[0]} {self.respan_env} has been created.")
            else:
                self.finished_signal.emit(False, "No valid Conda paths found to create the RESPAN environment.")

    def update_log_display(self, message):
        self.log_display.append(message)

    def find_conda_paths(self):
        """
        Find potential conda installation paths on the system.
        """
        paths = []
        if os.name == 'nt':  # Windows
            default_paths = [
                os.path.join(os.getenv('USERPROFILE'), 'Anaconda3'),
                os.path.join(os.getenv('USERPROFILE'), 'Miniconda3')
            ]
        ...
        # Check the default paths AND ignoreing miniforge (at least for now)
        for path in default_paths:
            if os.path.exists(path) and 'miniforge' not in path.lower():
                paths.append(path)

        # Check the PATH environment variable
        for path in os.getenv('PATH').split(os.pathsep):
            if 'conda' in path.lower() and 'miniforge' not in path.lower() and os.path.exists(path):
                paths.append(os.path.dirname(path))
        # Remove duplicates
        paths = list(set(paths))
        return paths

    def get_conda_envs(self, conda_path):
        """
        Get the list of conda environments from a given conda installation path.
        """
        try:
            # Determine the conda executable path
            if os.name == 'nt':  # Windows
                conda_executable = os.path.join(conda_path, 'Scripts', 'conda.exe')
            else:  # Unix-based systems
                conda_executable = os.path.join(conda_path, 'bin', 'conda')

            if not os.path.exists(conda_executable):
                self.logger.error(f"Conda executable not found in {conda_path}")
                return []

            result = subprocess.run([conda_executable, 'env', 'list'], capture_output=True, text=True)
            envs = []
            ...
            self.logger.error(f"Error retrieving environments from {conda_path}: {e}")
            return []

    def load_conda_env_info_print_list(self):
        conda_paths = self.find_conda_paths()
        for conda_path in conda_paths:
            self.logger.info(f"Conda path: {conda_path}")
            envs = self.get_conda_envs(conda_path)
            if envs:
                self.logger.info("Available environments:")
                for env in envs:
                    self.logger.info(f"  - {env}")
            else:
                self.logger.info("No environments found.")

    def create_respan_env(self, env_name, conda_path):
        self.logger.info(
            " RESPAN enviornment not found, a new enviornment will be created. Please allow ~15mins for download and installation of pytorch and ~3GB of necessary additoinal packages...")
        self.logger.info(
            " If enviornment creation is taking >30 minutes, you may need to update Anaconda within the Anaconda Navigator"
            "\n We also provide a working respan evironment, which you may download and unzip (using 7zip) via the link below:"
            " <respan download link>")

        self.logger.info(f" Anaconda found in {conda_path}. Creating environment '{env_name}'.")
        # Paths to the conda and pip executables
        conda_executable = os.path.join(conda_path, "Scripts", "conda.exe")
        pip_executable = os.path.join(conda_path, "Scripts", "pip.exe")

        try:
            # Commands to create the environment and install packages
            cmds = [
                (f'{conda_executable} update -n base conda',  # -c conda-forge --all -y',
                 " Updating conda... Please allow 2 minutes for update. This process may be prolonged if using an old installation of Anaconda"),
                (f'{conda_executable} create --name {env_name} python=3.9  -y',
                 " Creating RESPAN conda environment... Please allow 1 minute for environment creation."),
                # (f'{conda_executable} install -n {env_name} conda - forge::mamba -y',
                # "Installing mamba..."),
                (
                f'{conda_executable} run -n {env_name} {conda_executable} install pytorch torchvision pytorch-cuda=12.1 -c pytorch -c nvidia -y',
                "Installing PyTorch and related packages... Please allow 10 min for these to download and install assuming a fast internet connection."),
                (f'{conda_executable} run -n {env_name} {conda_executable} install scikit-image opencv -y',
                 "Installing additional packages... Please allow 2 min for these installations."),
                (f'{conda_executable} run -n {env_name} {pip_executable} install argparse', "Installing argparse..."),
                (f'{conda_executable} run -n {env_name} git clone -b v2.3.1 https://github.com/MIC-DKFZ/nnUNet.git',
                 "Cloning nnUNet 2.3.1 repository... Please allow 1 min for this to download."),
                (f'{conda_executable} run -n {env_name} {pip_executable} install -e ./nnUNet',
                 "Installing nnUNet from repository... Please allow 1 min for this to install."),
                # consider deleting to clean up

            ]
            clone_nnunet_repo_cmd = 'git clone https://github.com/MIC-DKFZ/nnUNet.git'
            install_nnunet_repo_cmd = f'{os.path.join(conda_path, "Scripts", "pip.exe")} install -e ./nnUNet'
            try:
                for cmd, msg in cmds:
                    ...
                    if process.returncode != 0:
                        self.logger.error(f"Error executing '{cmd}': {stderr.decode()}")
                        self.finished_signal.emit(False, f"Failed during: {msg}")
                        return
                    else:
                        self.logger.info(f"Successfully completed: {stdout.decode()}")

                self.logger.info(f"Environment '{env_name}' setup is complete.")
                self.finished_signal.emit(True, f"Environment '{env_name}' setup is complete.")

            except Exception as e:
                self.logger.error(f"Unexpected error: {e}")
                self.finished_signal.emit(False, f"Unexpected error during setup: {str(e)}")
...
class RESPANAnalysis(QWidget):
    ...
    ### added to load cusolver64_11.dll
    def load_dlls(self):

        import ctypes

        try:
            dll_dir = os.path.join(base_path, '_internal')
    ...
    def initUI(self):
        ...
        self.info_label = QLabel("RESPAN will process data stored in subfolders of the data directory.")
        ...
        self.Track_label = QLabel(
            "Each subfolder should contain a single neuron with sperate volumes for each timepoint.")
    ...
    def run_function(self):
        ...
        self.worker = AnalysisWorker(RESPAN, directory, model_dir, save_intermediate, save_validation,
...
class AnalysisWorker(QThread):
    ...
    def __init__(self, RESPAN, directory, model_dir, save_intermediate, save_validation,
                 inputxy, inputz, modelxy, modelz, neuron_ch, analysis_method,
    ...
    def run(self):
        ...
        try:
            ...
            from RESPAN.Environment import main, imgan, timer, strk
            main.check_gpu()
            ...
            for subfolder in os.listdir(self.directory):
                ...
                if os.path.isdir(subfolder_path):
                    ...
                    log_path = subfolder_path + 'RESPAN_Log' + datetime.now().strftime("%Y-%m-%d_%H-%M-%S") + '.log'
                    ...
                    settings.min_dendrite_vol = round(
                        self.min_dendrite_vol / settings.input_resXY / settings.input_resXY / settings.input_resZ, 0)
                    settings.neuron_spine_size = [
                        round(x / (settings.input_resXY * settings.input_resXY * settings.input_resZ), 0) for x in
                        self.spine_vol]
                    ...
                    self.logger.info(
                        f"  Spine volume set to: {self.spine_vol[0]} to {self.spine_vol[1]} µm<sup>3</sup>, {settings.neuron_spine_size[0]} to {settings.neuron_spine_size[1]} voxels")
                    self.logger.info(
                    ...
                    if os.path.exists(global_conda_path + '/envs/respan/'):
                        ...
                        self.logger.info(" Path set to:" + global_conda_path + '/envs/' + global_respan_env + '/')
                        self.logger.info(
...
class RESPANValidation(QWidget):
    def __init__(self):
        ...
        self.info_label2 = QLabel(
            "Ensure the correct Analysis_Settings.yaml file is included in the analysis output subfolder.")
        self.line2 = QFrame()
        self.line2.setFrameShape(QFrame.HLine)
        self.line2.setFrameShadow(QFrame.Sunken)
    ...
    def run_function(self):
        ...
        self.worker = ValidationWorker(RESPAN, ground_truth, analysis_output, neuron_ch, min_dendrite_vol, spine_vol,
                                       spine_dist, inputxy, inputz, self.logger)
        self.worker.task_done.connect(self.on_task_done)
        self.worker.start()
...
class ValidationWorker(QThread):
    ...
    def __init__(self, RESPAN, ground_truth, analysis_output, neuron_ch, min_dendrite_vol, spine_vol, spine_dist,
                 inputxy, inputz, logger):
        super().__init__()
        self.RESPAN = RESPAN
    ...
    def run(self):
        ...
        try:
            ...
            log_path = self.analysis_output + 'RESPAN_Validation_Log' + datetime.now().strftime(
                "%Y-%m-%d_%H-%M-%S") + '.log'
...
class UNet(QWidget):
    def __init__(self):
        ...
        self.datasetID_input.setText('055')
        ...
        # horizontal_input2.addWidget(self.conda_path_label)
        # horizontal_input2.addWidget(self.conda_path_input)
        # horizontal_input2.addWidget(self.respan_env_label)
        # horizontal_input2.addWidget(self.respan_env_input)
    ...
    def run_function(self):
        ...
        try:
            ...
            QMessageBox.critical(self, "Error", "Invalid input for conda path.")
            self.progress.setVisible(False)
            return
        ...
        try:
            ...
            QMessageBox.critical(self, "Error", "Invalid input for nnU-Net enviornment.")
            self.progress.setVisible(False)
            return
        '''
        ...
        # self.worker = Worker(RESPAN, directory, channel_options, integers, self.logger.get_logger())
        # self.worker = Worker(RESPAN, ground_truth, analysis_output, self.logger)
        # raw, preprocessed, results, datasetID, conda_dir,respan_env, logger

        self.worker = UNetWorker(RESPAN, input_dir, model_output, datasetID, ignore,
                                 global_conda_path, global_respan_env, self.logger)
        self.worker.task_done.connect(self.on_task_done)
        self.worker.start()
...
class UNetWorker(QThread):
    ...
    def run(self):
        ...
        try:
            ...
            if os.path.exists(global_conda_path + '/envs/' + global_respan_env + '/'):
                ...
                mt.train_nnUNet(parent_dir, preprocessed_folder, self.model_output, self.datasetID, global_conda_path,
                                global_respan_env, self.logger)
...
class CARE(QWidget):
    def __init__(self):
        ...
        self.patch_size_input = QLineEdit("8, 128, 128")
        self.patch_size_input.setFixedWidth(90)
        self.num_patches_per_img_input = QLineEdit("10")
        self.num_patches_per_img_input.setFixedWidth(90)
        self.epochs_input = QLineEdit("60")
        ...
        self.steps_per_epoch_input = QLineEdit("10")
...
class CAREWorker(QThread):
    ...
    def run(self):
        ...
        try:
            ...
            if not os.path.isdir(os.path.join(self.input_dir, "highSNR")) or not os.path.isdir(
                    os.path.join(self.input_dir, "lowSNR")):
                self.logger.info(
                    "Missing 'highSNR' or 'lowSNR' subfolders! \nPlease ensure both subfolders are present within the selected directory.\nRefer to guide as needed regarding data preparation.")
                ...
                log_path = self.input_dir + 'RESPAN_CARE_Training_Log' + datetime.now().strftime(
                    "%Y-%m-%d_%H-%M-%S") + '.log'
                self.logger.set_log_file(log_path)
                ...
                # if os.path.exists(settings.nnUnet_conda_path + '/envs/' + settings.respan_env + '/'):

                mt.train_care(self.input_dir, self.model_output, self.model_name, self.augmentation, self.data_type,
                              self.patch_size,
    ...
    def run_function(self):
        ...
        try:
            ...
            for dir_path, label in [(ground_truth, "training data directory"),
                ...
                # conda_path = self.conda_path_input.text()
                # respan_env = self.respan_env_input.text()
                if not os.path.exists(global_conda_path):
            ...
            self.worker = SelfNetWorker(RESPAN, input_dir, model_output, model_name,
                                        minv, maxv, minb, scale, xy_int, xz_int, batch_size, epochs, log_interval,
                                        imshow_interval,
                                        global_conda_path, global_respan_env, self.logger)
            self.worker.task_done.connect(self.on_task_done)
            self.worker.start()
...
class SelfNetWorker(QThread):
    ...
    def run(self):
        ...
        try:
            ...
            log_path = self.input_dir + 'RESPAN_Self-Net_Training_Log' + datetime.now().strftime(
                "%Y-%m-%d_%H-%M-%S") + '.log'
            self.logger.set_log_file(log_path)
            ...
            self.logger.info(f" Using conda environment: {global_conda_path} {global_respan_env}")
            ...
            if os.path.exists(self.conda_path + '/envs/' + self.respan_env + '/'):
                # log = imgan.restore_and_segment(settings, locations, self.logger)
                run_external_script(
                    "SelfNet_Model_Training.py", self.conda_path,
                    self.respan_env, args_dict, self.logger)
                self.logger.info(
                    "-----------------------------------------------------------------------------------------------------")
                self.logger.info("RESPAN Version: " + __version__)
                self.logger.info("Release Date: " + __date__ + "")
